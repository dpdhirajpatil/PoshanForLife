...
public interface InvoiceCounterRepository extends JpaRepository<InvoiceCounter, String> {
     ...
     */
    @Modifying
    @Query(value = "insert into invoice_counters (month_key, next_value) values (:monthKey, 1) "
            + "on conflict (month_key) do nothing", nativeQuery = true)
    void ensureCounterExists(@Param("monthKey") String monthKey);

    /** SELECT ... FOR UPDATE — serializes concurrent invoice-number generation. */
    @Lock(LockModeType.PESSIMISTIC_WRITE)
    @Query("select c from InvoiceCounter c where c.monthKey = :monthKey")
    Optional<InvoiceCounter> lockByMonthKey(@Param("monthKey") String monthKey);
}
