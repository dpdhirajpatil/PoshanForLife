 ...
 *
 * Products is deliberately NOT offered to this role, unlike every other graph: a LEAD is a
 * prospect in health-only mode, so the tab was removed rather than moved to an overflow.
 * Re-enabling means restoring the route constants, the tab entry, the `when` branch, and
 * the product-detail sibling route — see git history for the removed block.
 */
